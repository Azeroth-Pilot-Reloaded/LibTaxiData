...
def parse_args() -> argparse.Namespace:
    ...
    operations.add_parser("list", help="List every configured profile")
...
def main() -> int:
    ...
    profiles = load_profiles(root / "tools" / "profiles.json")
    if args.operation == "list":
        print_profiles(profiles)
        return 0
    if args.operation == "check":
        errors = catalog_errors(root, profiles)
        if errors:
            for error in errors:
                print(f"ERROR: {error}")
            return 1
        print(f"Client profile catalog is synchronized ({len(profiles)} profiles).")
        return 0

    if args.prune:
        for path in prune_profile_directories(root, profiles):
            print(f"Removed {path.relative_to(root)}")
    generate_client_profiles(root, profiles)
    update_toc(root, profiles)
    errors = catalog_errors(root, profiles)
    if errors:
        raise RuntimeError("\n".join(errors))
